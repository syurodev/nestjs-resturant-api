...
export class RestaurantBrandController {
  ...
  async create(
    ...
    let existingRestaurantBrand: RestaurantBrand = await this.restaurantBrandService.findWithRestaurantIdAndName(restaurantBrandDTO.restaurant_id, restaurantBrandDTO.name);
    ...
    let savedRestaurantBrand: RestaurantBrand = await this.restaurantBrandService.create(
      employee.id,
      restaurantBrandDTO
    ) as RestaurantBrand;
  ...
  async createMulti(
    ...
    let existingRestaurantBrands: RestaurantBrand[] = await this.restaurantBrandService.findNameArray(
      restaurantBrandDTO.map(item => item.restaurant_id),
      restaurantBrandDTO.map(item => item.name),
    )
    ...
    if (existingRestaurantBrands && existingRestaurantBrands.length > 0) {
      throw new HttpException(
        new ExceptionResponseDetail(
          HttpStatus.BAD_REQUEST,
          `Tên thương hiệu [${existingRestaurantBrands.map(item => item.name)}] này đã tồn tại!`
        ),
        HttpStatus.OK
      );
    }
    ...
    if (!savedRestaurantBrand || savedRestaurantBrand.length === 0) {
      response.setMessage(HttpStatus.BAD_REQUEST, "Tạo thương hiệu thất bại!");
      return res.status(HttpStatus.OK).send(response);
    }

    response.setData(new RestaurantBrandsResponse().mapToList(savedRestaurantBrand))
    return res.status(HttpStatus.OK).send(response);
  }
  ...
  async updateBrand(
    ...
    let existingRestaurantBrand: RestaurantBrand = await this.restaurantBrandService.findRestaurantBrandWithId(
      id,
      employee.id
    );
    ...
    let updatedRestaurantBrand: RestaurantBrand = await this.restaurantBrandService.update(existingRestaurantBrand);
  ...
  async brandDetail(
    ...
    response.setData(new RestaurantBrandDetailResponse(existingRestaurantBrand));
    return res.status(HttpStatus.OK).send(response);
  }
  ...
  async listBrand(
    ...
    response.setData(new RestaurantBrandsResponse().mapToList(
