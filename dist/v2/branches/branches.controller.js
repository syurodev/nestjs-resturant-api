...
let BranchesController = class BranchesController {
    ...
    async createMulti(branchesDTO, res, employee) {
        let response = new utils_response_common_1.ResponseData();
        response.setData(new branches_response_1.BranchesResponse().mapToList((await this.branchesService.createMulti(employee.id, branchesDTO))));
        return res.status(common_1.HttpStatus.OK).send(response);
    }
    async updateBrand(branchesUpdateDTO, res, id, employee) {
        let response = new utils_response_common_1.ResponseData();
        const updatedBranche = await this.branchesService.update(employee.id, id, branchesUpdateDTO.name);
